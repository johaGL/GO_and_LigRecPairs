...
## receiver, cell type expressing receptors 
receiver = "FAPs activated"
expressed_genes_receiver = get_expressed_genes(receiver, seuratObj, pct = 0.10)

background_expressed_genes = expressed_genes_receiver %>% .[. %in% rownames(ligand_target_matrix)]

## sender  : sender cells express ligands
sender_celltypes = c("FAPS, Smoc2+","Endothelial cells","unknown",
                  "Endothelial_lymphatic","FAPs, Smoc2+","Myonuclei","B_lymphocytes.CD4",
                  "MuSc","Tenocytes","Early T","FAPs?, Smoc2++",
                  "Schwann cells","?FAPs??","Schwann cells")  #I excluded "Smooth muscle", "Neutrophyls_macrophages", and off course "FAPs activated"
...
savedmarkers <- read.table(test,"testFAPpi16_abs0.8.txt", sep="\t", header =T)
geneset_oi1 <- savedmarkers$gene[savedmarkers$cluster==1 & savedmarkers$avg_logFC > 0.8]
...
pdf("ligandreceptorheatmap.pdf", width=12)
