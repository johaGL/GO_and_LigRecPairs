# ,,,,,,,,,,,,,,,,,,,,
# ,,,,,, CIRCOS ,,,,,,
# THIS IS VERSION SHOWS PI16 RECEPTORS
...
expressed_genes_allCells <- ALLMARKERS %>% 
  filter(celltypes %in% allCells & avg_logFC > 1) %>% pull(gene)
expressed_genes_allCells <- as.character(expressed_genes_allCells)


# Load gene set of interest and set background cells Other
...
cutoff_include_all_ligands = active_ligand_target_links_df$weight %>% quantile(0.33, na.rm = TRUE)
...
ligand_order = c(FAPSmoc2.main_specific_ligands, FAPSmoc2.others_specific_ligands,FAPpi16_specific_ligands,
                 tenocytes_specific_ligands,  endothelial_specific_ligands,
                  smooth_specific_ligands, schwann_specific_ligands,
                 musc_specific_ligands, neumacr_specific_ligands
                 ) %>% 
...
pdf("LigRecPairs/circos_pi16recepLegend.pdf",  width=15)
  ggplot(active_ligand_target_links_df_circos, aes(x=ligand_type, y=weight, fill=ligand_type))+ geom_violin()+
    scale_fill_manual(values= c("steelblue2", "orangered","darkorange","gold", 
                                "purple", "wheat2","turquoise4" , "lightcyan3", "deeppink4"))+
    ggtitle("Ligands weights as calculated by NicheNet")
dev.off()
...
# not working:
#   
#   width_same_cell_same_ligand_type = 0.05
# width_different_cell = 0.05
# width_ligand_receptor = 0.05
# width_same_cell_same_receptor_type = 0.05
#   
#   gaps = c(
...
#     rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type =="FAPSmoc2.others" ) %>% distinct(ligand) %>% nrow() -1)), 
#     width_different_cell,
#     rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "Tenocytes") %>% distinct(ligand) %>% nrow() -1)),
#     width_different_cell,
#     rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "Endothelial") %>% distinct(ligand) %>% nrow() -1)),
#     width_different_cell,
